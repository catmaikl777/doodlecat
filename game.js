class DoodleJumpGame {
    constructor() {
        ...
        this.sizes = {
            ...
            spring: { width: 20, height: 10 },
            rocket: { width: 30, height: 50 },
            monster: { width: 40, height: 40 },
        ...
        this.platformTypes = ['green', 'blue', 'white', 'brown', 'moving', 'breakable', 'spring', 'rocket'];
        this.itemTypes = ['coin', 'shield', 'magnet'];
    ...
    async loadPlatformSprites() {
        this.sprites.platforms.green = this.createPlatformSprite('#4CAF50', '#388E3C');
        this.sprites.platforms.blue = this.createPlatformSprite('#2196F3', '#1976D2');
        this.sprites.platforms.white = this.createPlatformSprite('#E0E0E0', '#BDBDBD');
        this.sprites.platforms.brown = this.createPlatformSprite('#8B4513', '#654321');
        this.sprites.platforms.moving = this.createPlatformSprite('#9C27B0', '#7B1FA2');
        this.sprites.platforms.breakable = this.createPlatformSprite('#FF9800', '#F57C00');
        this.sprites.platforms.spring = this.createPlatformSprite('#4CAF50', '#388E3C', '#FFC107');
        this.sprites.platforms.rocket = this.createPlatformSprite('#FF5722', '#D84315', '#FFD700');
    }
    ...
    createPlayerSprite(eyeOffset = 0, isJumping = false) {
        return new Promise((resolve) => {
            ...
            ctx.fillRect(leftEyeX + eyeSize * 0.3, eyesY + eyeSize * 0.3, pupilSize, pupilSize);
            ctx.fillRect(rightEyeX + eyeSize * 0.3, eyesY + eyeSize * 0.3, pupilSize, pupilSize);
    ...
    createPlatformSprite(color, darkColor, stripeColor = null) {
        ...
        const gradient = ctx.createLinearGradient(0, 0, 0, this.sizes.platform.height);
        ...
        if (stripeColor) {
            ...
            for (let i = spacing; i < this.sizes.platform.width - spacing; i += spacing) {
                ctx.fillRect(i, topHeight, stripeWidth, this.sizes.platform.height - topHeight);
            }
        }
    ...
    createCoinSprite() {
        ...
        const gradient = ctx.createRadialGradient(
            this.sizes.coin.width/2, this.sizes.coin.height/2, 0,
            this.sizes.coin.width/2, this.sizes.coin.height/2, this.sizes.coin.width/2
        );
        ...
        ctx.fillStyle = gradient;
        ctx.beginPath();
        ctx.arc(this.sizes.coin.width/2, this.sizes.coin.height/2, this.sizes.coin.width/2, 0, Math.PI * 2);
        ctx.fill();
        
        // Блики
        ctx.fillStyle = 'rgba(255, 255, 255, 0.6)';
        ctx.beginPath();
        ctx.arc(this.sizes.coin.width/2 - 3, this.sizes.coin.height/2 - 3, this.sizes.coin.width/4, 0, Math.PI * 2);
        ctx.fill();
    ...
    createShieldSprite() {
        ...
        // Щит
        const gradient = ctx.createRadialGradient(
            this.sizes.shield.width/2, this.sizes.shield.height/2, 0,
            this.sizes.shield.width/2, this.sizes.shield.height/2, this.sizes.shield.width/2
        );
        ...
        ctx.fillStyle = gradient;
        ctx.beginPath();
        ctx.arc(this.sizes.shield.width/2, this.sizes.shield.height/2, this.sizes.shield.width/2, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.arc(this.sizes.shield.width/2, this.sizes.shield.height/2, this.sizes.shield.width/2 - 2, 0, Math.PI * 2);
        ctx.stroke();
    ...
    generatePlatform(offsetY = 0) {
        const platformTypes = ['green', 'blue', 'green', 'green', 'spring', 'moving', 'breakable'];
    ...
    update() {
        if (this.localState.gameOver || !this.localState.connected || !this.spritesLoaded) return;
        ...
        if (this.player.y < this.cameraY) {
            ...
            if (this.platforms[this.platforms.length - 1].y > this.cameraY - this.canvas.height) {
        ...
        if (this.player.y > this.cameraY + this.canvas.height + 200) {
            if (this.localState.shieldActive) {
                ...
                this.createParticles(this.player.x + this.player.width/2, this.player.y + this.player.height/2, 15, '#4FC3F7');
    ...
    handlePlatformCollision(platform) {
        switch(platform.type) {
            case 'breakable':
                ...
                if (platform.health <= 0) {
                    ...
                    this.createParticles(platform.x + platform.width/2, platform.y, 10, '#8B4513');
                } else {
                    this.createParticles(platform.x + platform.width/2, platform.y, 3, '#FF9800');
                }
                break;
                
            case 'spring':
                this.player.velocityY = -20;
                this.createParticles(platform.x + platform.width/2, platform.y, 5, '#FFC107');
                break;
                
            case 'rocket':
                this.activateRocket();
                this.createParticles(platform.x + platform.width/2, platform.y, 8, '#FF5722');
                break;
        ...
        if (platform.type !== 'broken') {
            ...
            if (platform.y < this.player.y + this.cameraY) {
                ...
                this.showScorePopup(platform.x + platform.width/2, platform.y, points);
    ...
    updateItems() {
        ...
        this.items = this.items.filter(item => !item.collected && item.y < this.cameraY + this.canvas.height + 100);
    }
    ...
    collectItem(item) {
        ...
        switch(item.type) {
            case 'coin':
                ...
                this.createParticles(item.x + item.width/2, item.y + item.height/2, 5, '#FFD700');
                break;
            ...
            case 'shield':
                ...
                this.createParticles(item.x + item.width/2, item.y + item.height/2, 8, '#4FC3F7');
            ...
            case 'magnet':
                ...
                this.createParticles(item.x + item.width/2, item.y + item.height/2, 8, '#E91E63');
    ...
    calculatePoints(platform) {
        let basePoints = 10;
        switch(platform.type) {
            case 'spring': basePoints = 50; break;
            case 'rocket': basePoints = 100; break;
            case 'moving': basePoints = 20; break;
            case 'breakable': basePoints = 15; break;
        }
    ...
    draw() {
        ...
        // Рисуем платформы
        this.platforms.forEach(platform => {
            if (platform.y >= this.cameraY && platform.y <= this.cameraY + this.canvas.height) {
        ...
        // Рисуем предметы
        this.items.forEach(item => {
            if (!item.collected && item.y >= this.cameraY && item.y <= this.cameraY + this.canvas.height) {
                this.drawItem(item);
            }
        });
    ...
    drawPlatform(platform) {
        ...
        if (platform.type === 'breakable' && platform.health < 3) {
            ...
            this.ctx.fillRect(platform.x, platform.y - this.cameraY - 5, healthWidth, 3);
        }
    }
    ...
    drawParticles() {
        this.particles.forEach(particle => {
            ...
            if (particle.isText) {
                ...
                this.ctx.fillStyle = particle.color;
                this.ctx.beginPath();
                this.ctx.arc(particle.x, particle.y - this.cameraY, particle.size, 0, Math.PI * 2);
                this.ctx.fill();
            }
        });
    ...
    drawPlayers() {
        ...
        if (this.player.isRocket) {
            ...
            this.ctx.beginPath();
            this.ctx.moveTo(this.player.x + this.player.width/2, this.player.y - this.cameraY + this.player.height);
            this.ctx.lineTo(this.player.x + this.player.width/2 - 8, this.player.y - this.cameraY + this.player.height + flameLength);
            this.ctx.lineTo(this.player.x + this.player.width/2 + 8, this.player.y - this.cameraY + this.player.height + flameLength);
            this.ctx.closePath();
            this.ctx.fill();
        ...
        Object.values(this.gameState.players).forEach(player => {
            if (player.id !== this.gameState.myPlayerId && player.y >= this.cameraY && player.y <= this.cameraY + this.canvas.height) {
                this.ctx.fillStyle = player.color;
                this.ctx.fillRect(player.x, player.y - this.cameraY, this.sizes.player.width, this.sizes.player.height);
                
                // Счет игрока
                this.ctx.fillStyle = 'black';
                this.ctx.font = '12px Arial';
                this.ctx.textAlign = 'center';
                this.ctx.fillText(player.score, player.x + this.sizes.player.width/2, player.y - this.cameraY - 5);
            }
        });
    }
    ...
    drawUI() {
        ...
        if (this.localState.combo > 1) {
            ...
            this.ctx.textAlign = 'center';
            this.ctx.fillText(`Комбо x${this.localState.combo}!`, this.canvas.width/2, 30);
        }
        ...
        if (this.localState.rocketActive) {
            this.ctx.fillStyle = '#FF5722';
            this.ctx.textAlign = 'left';
            this.ctx.fillText(`Ракета: ${Math.ceil(this.localState.rocketTime/60)}с`, 15, powerUpY);
            powerUpY += 20;
        }
        ...
        if (this.localState.shieldActive) {
            ...
            this.ctx.textAlign = 'left';
            this.ctx.fillText(`Щит: ${Math.ceil(this.localState.shieldTime/60)}с`, 15, powerUpY);
            powerUpY += 20;
        }
        ...
        if (this.localState.magnetActive) {
            ...
            this.ctx.textAlign = 'left';
            this.ctx.fillText(`Магнит: ${Math.ceil(this.localState.magnetTime/60)}с`, 15, powerUpY);
        }
    }
    ...
    updateUI() {
        ...
        document.getElementById('player-count').textContent = Object.keys(this.gameState.players).length;
